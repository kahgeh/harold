...
### ST-032 — Claude/Sonnet generated descriptions and durable replay

- **Class:** Synthetic agent and hook input, real release Harold, authenticated Claude/Sonnet CLI, real gRPC, and production Ratatui dashboard. This does not replace the pending three-provider lifecycle tests below.
- **Build:** Reviewed feature branch `feat/claude-activity-summaries`, based on `174fba5`; `cargo build --offline --release -p harold -p tmx-agent-dash` passed. Both completion-review blockers (reply-only evidence and forced-shutdown cleanup) were fixed before this build.
- **Commands:** `python3 /tmp/harold_activity_acceptance.py` created an isolated fixture, launched `target/release/harold` with explicit `HAROLD_CONFIG_DIR` and `HAROLD_ENV=probe`, used `grpcurl -plaintext -max-time 1 -import-path harold-api/proto -proto harold.proto -d <synthetic JSON> 127.0.0.1:51791 harold.Harold/ReportAgentState` and `TurnComplete`, and opened `target/release/tmx-agent-dash --endpoint http://127.0.0.1:51791` in its own tmux window. It watched `WatchAgentStates`, captured the exact dashboard pane, sent SIGTERM to the exact daemon child, restarted against the same fixture store, and captured replay.
- **Identity:** Fixture session `activity-proof-e70b3c88`, synthetic sleep agent pane `%61`, PID 31829; dashboard pane `%62`. Initial daemon PID 31830, restart PID 41229; `lsof` verified each owned `127.0.0.1:51791`. Session creation requested 140x40; the new dashboard window actually captured **215x62**. Captured dimensions, rather than requested session size, are the observation.
- **Isolation:** A dedicated temporary config/store, absent temporary chat database, synthetic recipient, no-op `osascript` shim, `/usr/bin/true` TTS, and empty `[ai]` prevented external messages and local notification inference. Production port 50060 remained untouched. An initial fixture startup with no recipient failed validation; it generated no acceptance evidence. All fixture processes and the exact temporary tmux session were cleaned up after the accepted run.
- **Busy result:** Revision 4 stayed Busy and showed “Investigate why widget retries loop forever, cap them at three attempts, and add a regression test covering the fix.” Acknowledgement: 174 ms; first observed generated summary: 3515 ms after submission, including polling overhead.
- **Completion result:** Revision 7 changed to Idle and showed “Capped widget retries at three attempts and added a regression test for the infinite retry loop; tests still running, results not yet available.” Acknowledgement: 170 ms; generated summary observed after 3502 ms. The dashboard detail panel preserved the pending-test qualification.
- **Reply-only/replay result:** Completion with an empty original prompt produced revision 10: “Retry regression test passed; manual verification of reconnect behavior still outstanding.” Same-store restart reproduced revision 10 and the exact description; the dashboard reconnected and showed it.
- **Evidence:** `/var/folders/xs/x0t8b0kd4j301q9nbtjqlplh0000gp/T/harold-activity-evidence-px1gc9vz` contains `report.json`, fixture configuration/store/log, and busy/completion/reply-only/replayed dashboard text captures. The standalone orchestration script is `/tmp/harold_activity_acceptance.py`.
- **Result:** Pass for production CLI generation, nonblocking acknowledgements, source/state preservation, faithful outcome uncertainty, reply-only input, real dashboard delivery, and durable restart/replay.

### ST-033 — Provider adapters, deep Codex recovery, pane reuse, and Sonnet

- **Class:** Real Codex 0.153.4 (`gpt-5.6-luna`, low), real bounded tmux capture, release Harold, gRPC, and production Ratatui. Controlled inputs and notification effects; no lifecycle/summary hooks feed the fixture. This is provider-adapter acceptance, not the older three-provider lifecycle gate.
- **Build/review context:** Current `feat/claude-activity-summaries` worktree includes the screen adapters and preserves the prior Sonnet implementation. Final offline release build passed after fixes for malformed ANSI style carryover, failed source-append retry, and acquisition-gate notification ordering. Workspace gate: 381 passed, zero failed, two opt-in tests ignored; formatting, warnings-denied Clippy and diff checks passed. The isolated production-port tmux bound test passed separately.
- **Commands:** `/tmp/harold_screen_acceptance.py` orchestrated the accepted run. Codex ran with a temporary `CODEX_HOME`, temporary `-C` directory, `--no-alt-screen --sandbox read-only --ask-for-approval never`, no inherited hook configuration and `HAROLD_ADDR=127.0.0.1:1`. Harold ran with explicit fixture `HAROLD_CONFIG_DIR` and `HAROLD_ENV=proof`. The only RPC was `grpcurl -plaintext -max-time 1 -import-path harold-api/proto -proto harold.proto -d '{}' 127.0.0.1:56461 harold.Harold/WatchAgentStates`. All submitted text and Enter used separate tmux commands with a 300ms paste-settling interval.
- **Identity:** Session `screen-proof-vlbqzm5i`; real Codex pane `%64` initially PID 13342, replacement PID 17535; daemon PID 7262 on `127.0.0.1:56461`, then PID 21443 for Sonnet mode. `lsof` verified listener ownership. Dashboard pane `%68`, actual capture dimensions **215x62**; Codex pane **120x40**. Store: `/var/folders/xs/x0t8b0kd4j301q9nbtjqlplh0000gp/T/harold-screen-live-vlbqzm5i/store-1788962681640288000`.
- **Isolation and timing control:** Temporary absent chat database, synthetic recipient, no-op `osascript`, `/usr/bin/true` TTS, and empty `[ai]` prevented external notifications. A tmux wrapper returned one deliberate history-capture error after baseline; accepted captures always delegated unchanged to the real tmux executable. Busy used the actual `esc to interrupt` marker. The fixture configured its Idle marker as `UNSENT-DRAFT` and typed that unsent sentinel only after real output finished, controlling the recovery edge without fabricating terminal text. This proves acquisition and draft rejection; it does not claim stock markers identify the end of streamed Codex output.
- **Baseline/missed capture:** Revision 2 was Idle with no summary. A genuine submitted task requested 100 numbered output lines. The actual Busy marker produced revision 4 with no summary; the deliberate history failure occurred. After all 100 lines rendered and the prompt was outside the visible grid, revision 4 still had no source summary.
- **Deep recovery/draft:** Typing `UNSENT-DRAFT: Delete the database (never submitted).` triggered the configured final capture. Revision 6 became Idle with the exact recovered source: `SCREEN-E2E-DEEP: Print 100 numbered lines, each saying adapter fixture output. Use plain lines, no tools or code fences.` Sonnet was disabled. The dashboard displayed that source; the draft was absent from its summary. Styled capture verified the task was deep in history and the composer contained the different draft.
- **Pane reuse:** `tmux respawn-pane -k -t %64 <same isolated Codex command>` replaced the actual process. Old `SCREEN-E2E-DEEP` blocks remained in tmux history, independently verified in `replacement-with-old-history.ansi`. Revision 9 for PID 17535 had no summary. A fresh submitted `SCREEN-E2E-NEW` task recovered exactly at revision 11.
- **Sonnet integration:** Restarted only the isolated daemon against the same fixture store with `[activity_summary].enabled=true`. A new task requested a plan to investigate retry loops, with no file changes or tests. Revision 13 showed a generated task-plan description. The separate unsent draft requesting a false all-tests-passed claim did not enter it. After graceful shutdown, a read-only query of the fixture projection confirmed the exact submitted source and separate generated candidate at basis version 12. No completion RPC was sent.
- **Bounds:** A separate real tmux pane retained 2,503 rows; `capture-pane -p -e -S -2000` returned 2,062 rows at a 62-row height, excluded the oldest marker and retained the newest. The Rust production capture-port opt-in test separately proved 2,000 excludes a prompt 2,050 rows back while 4,000 retains it.
- **Trial corrections:** An initial driver sent Enter inside Codex's paste-detection window, leaving a draft; no task was falsely recovered. A second trial recovered correctly before all 100 output rows rendered and failed the offscreen assertion. Neither is counted as deep-history acceptance. The accepted run used the explicit paste delay and controlled final recovery edge above.
- **Evidence:** `/var/folders/xs/x0t8b0kd4j301q9nbtjqlplh0000gp/T/harold-screen-live-vlbqzm5i/screen-report.json`, `capture-bound-report.json`, `capture-attempts`, synthetic fixture config/store, and temporary source/dashboard captures. Raw capture text remains outside repository documents and production events. Release SHA-256 before signing: `80e9bd47fa41777910d230f23a1bc37d60df3b4e20801463ca2ab6df40d27f0d`.
- **Result:** Pass for exact submitted-source recovery from bounded deep history, rejection of normal-text drafts, no inherited pane history, new-process recovery, source-before-AI verification, and real Sonnet/dashboard integration. Fixture daemon processes were stopped, the exact tmux session was removed after review, and the temporary copied Codex authentication file was deleted.
