...
- [x] Current-code same-store acceptance at `1568fd0`: the first snapshot at event version 602 had 10 rows and zero exact placeholders; after a graceful `%6` restart, event version 604 again had 10 rows and zero, and dashboard `%26` showed rev604 with no placeholder occurrence.
...
- [x] Commit only the owned durable docs and task record as `docs: document agent state monitoring` (`c0559c8`).
...
- [x] Restart isolated `50061` on corrected code `1568fd0` from `%6` with the fixture configuration and repeat the same-store placeholder/replay proof.
