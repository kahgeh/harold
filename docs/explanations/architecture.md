...
1. `skip_if_session_active = true` (default) → skip if completing pane is in the active tmux session
1a. `skip_if_pane_active = false` → skip if completing pane is the active pane and screen is unlocked
