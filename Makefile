...
IDENTITY   := $(shell cat .codesign-identity 2>/dev/null)
...
setup-codesign .codesign-identity:
	@bash scripts/setup-codesign.sh

deploy: build .codesign-identity
	mkdir -p $(DEPLOY_DIR)
	cp $(BINARY) $(DEPLOY_DIR)/harold
	codesign --force --sign "$(IDENTITY)" $(DEPLOY_DIR)/harold
